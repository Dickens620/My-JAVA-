...
public class HashTable {
    public static void main(String[] args) {
        ...
        // It doesn't exist if the index <0 or index >10000 or location in the hashTable
        // is
        // null
    ...
    /**
     * Method to extract all words(only valid words) into a hash table(array). Here
     * valid words are, words consisting of only alphabets It returns the hash
     * table.
     */
    public static String[] createHashTable(String fileName) {
        ...
        String[] bucket = new String[10000000];
        ...
        try (Scanner scanner = new Scanner(new File(fileName));) {
            while (scanner.hasNextLine()) {
                // Read line by line and extract the individual words using the getWordsFromLine
                // method
    ...
    public static int generateHash(String word) {
        ...
        for (int i = 0; i < letters.length; i++) {
            ...
            totalSum = totalSum + (int) (alphabet.indexOf(letters[i]) * Math.pow(2, i));
        }
        return totalSum;
    }
}
